...
class Files_Admin{
    ...
    private function getUiSidebar($mainPath){
        $ui = new UI();
        $fileSystem = new Files_System($mainPath);
        $ui->setLayout(Files_System::UI_FILES_LIST);
        if(!empty($_POST['path'])){
        ...
        $ui->addVariable(Files_System::UI_CONTENT, $fileSystem->getFolderContent());
        return $ui->process();
    }
    ...
    {
        $ui = new UI();
        $ui->setLayout(Files_System::UI_MANAGER);
    ...
    public function adminDeleteFile()
    {
        ...
        $fileSystem = new Files_System($root);
