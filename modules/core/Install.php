...
class Core_Install implements Module_IInstall{
    ...
    private function installModule($moduleName)
    {
        if($this->isModuleExistLocally($moduleName)) {
            if($this->isModuleHasInstallation($moduleName) && !in_array($moduleName, $this->installedModules)) {
                /* @var $moduleInstallObject Module_IInstall */
     ...
     * Get install object
     * @param $moduleName string
     * @return Module_IInstall
     */
    protected function getInstallObject($moduleName) {
        $class = ucfirst($moduleName)."_Install";
        return new $class;
    }
    ...
    {
        return array(
            new Module_Dependency('module'),
            new Module_Dependency('page'),
            new Module_Dependency('user'),
            new Module_Dependency('access')
        );
    }
    /**
     * Try to resolve module dependency
     * @param $dependency Module_Dependency
    ...
    {
        $file = Core::MODULES_FOLDER.strtolower($moduleName)."/Install.php";
        return is_file($file);
    }
